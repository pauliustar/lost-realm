...
bool Quest_Context::DoAction(const EOPlus::Action& action)
{
	...
	else if (function_name == "addnpcbox")
	{
		std::string boxtitle = action.expr.args[0];
		std::string boxmessage = action.expr.args[1];
	...
	else if (function_name == "giveexp")
	{
	    ...
	    int online = this->character->world->characters.size();

	    if (online >= 5 && online < 15)
	    {
	    	int bonus_exp = 0.2 * int(action.expr.args[0]);
	    	this->character->exp = this->character->exp + (bonus_exp + int(action.expr.args[0]));
	    	this->character->StatusMsg("You gained " + util::to_string(bonus_exp) + " bonus experience [5+ Online Players Streak]");
	    }
	    else if (online >= 15)
	    {
	    	int bonus_exp = 0.4 * int(action.expr.args[0]);
	    	this->character->exp = this->character->exp + (bonus_exp + int(action.expr.args[0]));
	    	this->character->StatusMsg("You gained " + util::to_string(bonus_exp) + " bonus experience [15+ Online Players Streak]");
	    }
	    else
	    {
		...
		PacketBuilder builder(PACKET_RECOVER, PACKET_REPLY, 17);
		...
		if (level_up)
		{
			...
			this->character->tp = this->character->maxtp;
            this->character->CalculateStats();
            this->character->PacketRecover();
	...
	else if (function_name == "resetstats")
	{
	...
	else if (function_name == "givefame")
	{
		int fame = this->character->fame;
		int bestfame = this->character->bestfame;

		int newFame = fame + int(action.expr.args[0]);

		if (newFame >= 100000)
		{
			this->character->fame = 99999;

			if (newFame > bestfame)
			{
				this->character->bestfame = newFame;
			}			
		} 
		else
		{
			this->character->fame = newFame;
	...
	else if (function_name == "removefame")
	{
		int fame = this->character->fame;

		if(fame <= int(action.expr.args[0]))
		{
			this->character->fame = 0;
		}
		else
		{
			int newFame = fame - int(action.expr.args[0]);
	...
	{
		int extra_hp = static_cast<int>(this->character->maxhp * (static_cast<double>(int(action.expr.args[0]) / 100.0)));

		this->character->map->QuestHeal(this->character, 4, extra_hp);

		this->character->DelItem(14, int(action.expr.args[1]));

		PacketBuilder reply(PACKET_ITEM, PACKET_JUNK, 11);
		reply.AddShort(14);
		reply.AddThree(1); // Overflows, does it matter?
		reply.AddInt(this->character->HasItem(14));
		reply.AddChar(this->character->weight);
		reply.AddChar(this->character->maxweight);
		this->character->Send(reply);
	}
	else if (function_name == "recycle")
	{
	...
	else if (function_name == "warp")
	{
        character->Warp(int(action.expr.args[0]), int(action.expr.args[1]), int(action.expr.args[2]), WARP_ANIMATION_ADMIN);
	}
...
bool Quest_Context::CheckRule(const EOPlus::Expression& expr)
{
	...
	else if (function_name == "partymembers")
	{
		int party_members = 0;
	    if (this->character->party) // Party kills
	    {
            UTIL_FOREACH(this->character->party->members, member)
            {
            	party_members++;
            }
			if(party_members >= int(expr.args[0]))
	        {
	        	return true;
	        }
	        else
	...
	else if (function_name == "hasfame")
	{
		return this->character->fame >= int(expr.args[0]);
	}
	else if (function_name == "hashealth")
	{
		int benefit = static_cast<int>(this->character->maxhp * (static_cast<double>(int(expr.args[0])) / 100.0));

		return this->character->hp < benefit;
	...
	else if (function_name == "notwearing")
	{
		return std::find(UTIL_CRANGE(this->character->paperdoll), int(expr.args[0])) == this->character->paperdoll.end();
	}	
