...
void world_playerevents(void *world_void)
{
    World *world = static_cast<World *>(world_void);

    int online = world->characters.size();

    if (online >= 5 && online < 15)
    {
    	world->ServerMsg("[Online Players Streak] - " + util::to_string(online) + " - Bonus 0.2x EXP from all Contracts!");
    }
    else if (online >= 15)
    {
    	world->ServerMsg("[Online Players Streak] - " + util::to_string(online) + " - Bonus 0.4x EXP from all Contracts!");
    }

}

void world_deathescape(void *world_void)
{
    World *world = static_cast<World *>(world_void);

    world->EscapeDeath = false;

    world->PortalEffect();
    world->EndEscapeSequence();

    UTIL_FOREACH(world->characters, character)
    {
    	if(character->mapid == 1)
    	{
			...
			character->ServerMsg("Portal is open! You have 30 seconds to escape, hurry!");
    	}
    }
}


void world_enddeathescape(void *world_void)
{
    World *world = static_cast<World *>(world_void);

	world->deathcounter = 0;

	UTIL_FOREACH(world->characters, character)
    {
    	if(character->mapid == 1)
    	{
			character->ServerMsg("Portal vanished. Collect energy from souls to bring it back!");
    	}
    }
}

void world_portal(void *world_void)
{
    World *world = static_cast<World *>(world_void);
...
// Spawn Dungeon NPC
bool World::SpawnDungeonNPC(int id, int mapid, int mapx, int mapy, int speed)
{
    unsigned char npc_index = this->GetMap(mapid)->GenerateNPCIndex();
    ...
    NPC *npc = new NPC(this->GetMap(mapid), id, mapx, mapy, speed, 1, npc_index, true);
    this->GetMap(mapid)->npcs.push_back(npc);
	npc->Spawn();
...
// Timer Instant
void world_startdungeon(void *world_void)
{
    World *world = static_cast<World *>(world_void);

    if (world->DungeonEnabled == false && world->SecretDungeon == false)
    {

	    world->ServerMsg("[Dungeon] " + std::string(world->dungeons_config[util::to_string(world->currentdungeon) + ".Name"]) + " is now active! 30 seconds until start. Type #join to participate! [Req: Level " + util::to_string(static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Level"])) + "]");

	    world->DungeonEnabled = true;

	    world->StartDungeon();

	    world->dmapid = world->currentdungeon;

	    world->wave = 1;

	    world->DisconnectTimerSeq = false;
    } 

}

// Timer 30s after call world->StartDungeon();
void world_dungeontimer(void *world_void)
{
    World *world = static_cast<World *>(world_void);

    world->SecretDungeon = true;

    for (int i = 1; util::to_string(static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Wavenpc" + util::to_string(i)])) != "0"; i++)
    {
    	int npc = static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Wavenpc" + util::to_string(i)]);
    	int wavex = static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Wavex" + util::to_string(i)]);
    	int wavey = static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Wavey" + util::to_string(i)]);
    	int wavespeed = static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Wavespeed" + util::to_string(i)]);
    	int waveamount = static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Waveamount" + util::to_string(i)]);
    	for (int j = 1; j <= waveamount; j++)
			world->SpawnDungeonNPC(npc, world->currentdungeon, wavex, wavey, wavespeed);
    }

}

// Timer 10s after call world->FloorTimer();
void world_switchfloor(void *world_void)
{
    World *world = static_cast<World *>(world_void);

    if (world->wave == world->nextdwaveid)
    {
    	world->SwitchingFloor = false;
    	UTIL_FOREACH(world->characters, character)
    	{
	    	if (character->mapid == static_cast<int>(world->dungeons_config[util::to_string(world->nextdmapid) + ".Previousmap"]))
	    	{
	    		character->Warp(static_cast<int>(world->dungeons_config[util::to_string(world->nextdmapid) + ".Startm"]), static_cast<int>(world->dungeons_config[util::to_string(world->nextdmapid) + ".Startx"]), static_cast<int>(world->dungeons_config[util::to_string(world->nextdmapid) + ".Starty"]), world->config["WarpBubbles"] ? WARP_ANIMATION_ADMIN : WARP_ANIMATION_NONE);
	    	}
    	}
    }
    else
    {
    	...
    	UTIL_FOREACH(world->characters, character)
    	{
    		if (character->mapid == static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Lastwavemap"]))
    		{
    			character->Warp(2, 9, 11, world->config["WarpBubbles"] ? WARP_ANIMATION_ADMIN : WARP_ANIMATION_NONE);
    		}
    	}
    }
}

// Timer 120s after call world->DisconnectTimer();
void world_disconnected(void *world_void)
{
    World *world = static_cast<World *>(world_void);

    Map *maps = world->GetMap(world->dmapid);
	...
	if (world->charactercounter == 0 && world->DisconnectTimerSeq == true)
    {
	    for (int i = 0; i <= 5; i++)
	    {
	    ...
	    world->DungeonEnabled = false;
	    world->SecretDungeon = false;
	    world->DisconnectTimerSeq = false;
	    world->dmapid = 0;

	    world->ServerMsg("[Dungeon] " + std::string(world->dungeons_config[util::to_string(world->currentdungeon) + ".Name"]) + " has ended.");
    }
}

void world_disconnectedflag(void *world_void)
{
    World *world = static_cast<World *>(world_void);

    world->DisconnectTimerSeq = true;
    world->DisconnectTimer();
}
...
// Timer repeatable forever
void world_dungeon(void *world_void)
{
	World *world = static_cast<World *>(world_void);

	std::list<int> allowed_list = ExceptUnserialize(world->dungeons_config["DungeonMaps"]);

    UTIL_FOREACH(world->characters, character)
    {
    	if (std::find(allowed_list.begin(), allowed_list.end(), character->mapid) != allowed_list.end())
        {	
	        if (character->indungeon == false && world->SecretDungeon == false)
	        {
	        	if(world->dmapid == 0 && world->SwitchingFloor != true)
	        	{
	        		character->DeathRespawn();
		        	//character->Warp(2, 9, 11, world->config["WarpBubbles"] ? WARP_ANIMATION_ADMIN : WARP_ANIMATION_NONE);
		        	character->ServerMsg("You did not return to dungeon in time and got killed in the fight!");	 
	        	}
	       	}
	       	else if (character->indungeon == false && world->SecretDungeon == true)
	       	{
	       		if(std::find(allowed_list.begin(), allowed_list.end(), world->dmapid) != allowed_list.end() && character->mapid == world->dmapid)
	       		{
	       			character->indungeon = true; 
	       		}
				else if(std::find(allowed_list.begin(), allowed_list.end(), world->dmapid) != allowed_list.end() && character->mapid != world->dmapid)
				{
		        	character->Warp(static_cast<int>(world->dungeons_config[util::to_string(world->dmapid) + ".Startm"]), static_cast<int>(world->dungeons_config[util::to_string(world->dmapid) + ".Startx"]), static_cast<int>(world->dungeons_config[util::to_string(world->dmapid) + ".Starty"]), world->config["WarpBubbles"] ? WARP_ANIMATION_ADMIN : WARP_ANIMATION_NONE);
		        	character->indungeon = true; 
				}     		
	       	}
    	}
	...
	if(world->DungeonEnabled == true && world->SecretDungeon == true)
	{
		Map *maps = world->GetMap(world->dmapid);

	    UTIL_FOREACH(world->characters, character)
	    {
	        if (character->mapid == static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Joinm"]))
	        {
	        	character->Warp(static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Startm"]), static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Startx"]), static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Starty"]), world->config["WarpBubbles"] ? WARP_ANIMATION_ADMIN : WARP_ANIMATION_NONE);
	        	character->ServerMsg("Floor: 1 - Fight!");
	        }
	    }
	    world->DungeonEnabled = false;
    }

    if(world->DungeonEnabled == false && world->SecretDungeon == true)
    {
    	...
    	if(world->SwitchingFloor == false)
    	{
        	...
        	if(world->counter == 0)
	    	{
	    		world->SwitchingFloor = true;
	    		world->wave++;
	    		world->nextdwaveid = static_cast<int>(world->dungeons_config[util::to_string(world->dmapid) + ".Nextwaveid"]);
	    		world->nextdmapid = static_cast<int>(world->dungeons_config[util::to_string(world->dmapid) + ".Nextwavemap"]);
	    		world->FloorTimer();
	    	}
    	}

    	if(world->counter == 0 && world->SwitchingFloor == true)
    	{	    	
    		if(world->wave == world->nextdwaveid && world->dmapid == static_cast<int>(world->dungeons_config[util::to_string(world->dmapid) + ".Startm"]) && world->dmapid != world->nextdmapid)
    		{  			
    			world->dmapid = world->nextdmapid;
	    		UTIL_FOREACH(maps->characters, character)
	            {
					character->ServerMsg("Floor: " + util::to_string(world->wave - 1) + " - Cleared! Moving to next floor in 10 seconds. Type #leave to exit now.");
	            }

			    for (int i = 1; util::to_string(static_cast<int>(world->dungeons_config[util::to_string(world->dmapid) + ".Wavenpc" + util::to_string(i)])) != "0"; i++)
			    {
			    	int npc = static_cast<int>(world->dungeons_config[util::to_string(world->dmapid) + ".Wavenpc" + util::to_string(i)]);
			    	int wavex = static_cast<int>(world->dungeons_config[util::to_string(world->dmapid) + ".Wavex" + util::to_string(i)]);
			    	int wavey = static_cast<int>(world->dungeons_config[util::to_string(world->dmapid) + ".Wavey" + util::to_string(i)]);
			    	int wavespeed = static_cast<int>(world->dungeons_config[util::to_string(world->dmapid) + ".Wavespeed" + util::to_string(i)]);
			    	int waveamount = static_cast<int>(world->dungeons_config[util::to_string(world->dmapid) + ".Waveamount" + util::to_string(i)]);
    				for (int j = 1; j <= waveamount; j++)
						world->SpawnDungeonNPC(npc, world->nextdmapid, wavex, wavey, wavespeed);
			    }

	    	}
    		if(world->wave > static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Maxwaves"]))
    		{
    			UTIL_FOREACH(world->characters, character)
	            {
	            	if (character->mapid == static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Lastwavemap"]))
	            	{
	            		world->FloorTimer();
	            		int fame = static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Famereward"]);
	            		character->fame = character->fame + fame;

	            		if (character->fame > character->bestfame)
	            		{
	            			character->bestfame = character->fame;
	            		}

	            		character->ServerMsg("Dungeon completed! " + util::to_string(fame) + " Fame Gained. Exiting " + std::string(world->dungeons_config[util::to_string(world->currentdungeon) + ".Name"]) + " in 10 seconds.");	            		
					    for (int i = 1; util::to_string(static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Reward" + util::to_string(i)])) != "0"; i++)
					    {
						    int reward = static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Reward" + util::to_string(i)]);
						    int amount = static_cast<int>(world->dungeons_config[util::to_string(world->currentdungeon) + ".Rewardamount" + util::to_string(i)]); 

	    					const EIF_Data& item = character->world->eif->Get(reward);					    	
		            		character->ServerMsg("You have found x" + util::to_string(amount) + " " + item.name);					    	
						    character->GiveItem(reward, amount);
					    }	            		
	            		character->indungeon = false;
	            	}
	            }
    	...
    	if(world->SwitchingFloor == false && world->SecretDungeon == true)
		{
			UTIL_FOREACH(maps->characters, character)
	        {
	            if (character)
	        ...
	        {
	        	if(world->DisconnectTimerSeq == false)
	        	{
	        		world->DisconnectChecker();
	        	}
	        }
	    }
    }
...
[false] - Stops "charging" of the portal. (NPC::Killed)
[true] - All counting added until portal opens & closes

*/

void world_death(void *world_void)
{
	World *world = static_cast<World *>(world_void);
	...
	UTIL_FOREACH(world->characters, character)
    {
    	...
    	if (character->mapid == 1)
    	{
	    	if(world->EscapeDeath == true)
			{
				if(world->deathcounter >= 5)
				{
					world->StartEscapeSequence();
					break;
				}
			}
			else if(world->EscapeDeath == false && world->deathcounter >= 5)
			{
				if(character->mapid == 1 && character->x == 16 && character->y == 13)
				{
					character->Warp(2, 18, 7, WARP_ANIMATION_ADMIN);
				}
			}
    	}
    	if (character->mapid == 1 && world->EscapeDeath == false && world->deathcounter == 0 && alive > 0)
    	{  		
			world->EscapeDeath = true;
			break;
    	}
    }
	...
	, admin_count(0)
{
	...
	this->gearbox_config.Read(this->config["GearboxFile"]);	
	this->giftbox_config.Read(this->config["GiftboxFile"]);	
	this->socket_config.Read(this->config["SocketsFile"]);
	this->recycle_config.Read(this->config["RecycleFile"]);
	this->dungeons_config.Read(this->config["DungeonsFile"]);
	this->mining_config.Read(this->config["MiningFile"]);
	this->DungeonEnabled = false;
	this->SecretDungeon = false;
	this->SwitchingFloor = false;
	...
	event = new TimeEvent(world_playerevents, this, 3600.00, Timer::FOREVER);
	this->timer.Register(event);

	event = new TimeEvent(world_dungeon, this, 0.5, Timer::FOREVER);
    this->timer.Register(event);

    event = new TimeEvent(world_death, this, 1.0, Timer::FOREVER);
    this->timer.Register(event);
...
void World::LoadMine()
{
    ...
    // Assuming `mining_config` is a map with the format key -> value.
    UTIL_FOREACH(this->mining_config, hc)
    {
        Mine_Drop *drop(new Mine_Drop);
        drop->item = util::to_int(hc.first);
        drop->chance = util::to_float(hc.second);
...
Character *World::CreateCharacter(Player *player, std::string name, Gender gender, int hairstyle, int haircolor, Skin race)
{
	...
	this->db.Query("INSERT INTO `characters` (`name`, `account`, `gender`, `hairstyle`, `haircolor`, `race`, `inventory`, `bank`, `weaponbank`, `shieldbank`, `cosmeticsbank`, `paperdoll`, `spells`, `quest`, `vars`@) VALUES ('$','$',#,#,#,#,'$','', '', '', '', '$','$','',''@)",
...
void World::DungeonTimer()
{
    TimeEvent *event = new TimeEvent(world_startdungeon, this, 1, 1);
    this->timer.Register(event);
}

void World::StartDungeon()
{
    TimeEvent *event = new TimeEvent(world_dungeontimer, this, 30, 1);
    this->timer.Register(event);
}

void World::FloorTimer()
{
    TimeEvent *event = new TimeEvent(world_switchfloor, this, 10, 1);
    this->timer.Register(event);
}
void World::DisconnectTimer()
{
    TimeEvent *event = new TimeEvent(world_disconnected, this, 120, 1);
    this->timer.Register(event);
}

void World::DisconnectChecker()
{
    TimeEvent *event = new TimeEvent(world_disconnectedflag, this, 1, 1);
    this->timer.Register(event);
}

void World::StartEscapeSequence()
{
    TimeEvent *event = new TimeEvent(world_deathescape, this, 0.5, 1);
    this->timer.Register(event);
}
void World::EndEscapeSequence()
{
    TimeEvent *event = new TimeEvent(world_enddeathescape, this, 30, 1);
    this->timer.Register(event);
}
void World::PortalEffect()
{
    TimeEvent *event = new TimeEvent(world_portal, this, 3, 10);
    this->timer.Register(event);
}