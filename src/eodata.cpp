...
void pub_read_record(EIF_Data& newdata, char* buf)
{
	...
	newdata.element = static_cast<EIF::Element>(PacketProcessor::Number(buf[53]));
	newdata.element_attack = PacketProcessor::Number(buf[54]);
...
/*
	newdata.element_a_type = static_cast<ENF::Element>(PacketProcessor::Number(buf[27]));
	newdata.element_power = PacketProcessor::Number(buf[29]); //
	newdata.element_w_type = static_cast<ENF::Element>(PacketProcessor::Number(buf[31]));
	newdata.element_weakness = PacketProcessor::Number(buf[33]);
*/

	newdata.element_attack = PacketProcessor::Number(buf[27], buf[28]);
	newdata.element_attack_dmg = PacketProcessor::Number(buf[29], buf[30]);
	newdata.element_weakness = PacketProcessor::Number(buf[31], buf[32]);
	newdata.element_weakness_dmg = PacketProcessor::Number(buf[33], buf[34]);
