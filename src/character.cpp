	...
	, display_cha(this->world->config["UseAdjustedStats"] ? adj_cha : cha)
{
	...
	"`karma`, `sitting`, `hidden`, `bankmax`, `goldbank`, `usage`, `inventory`, `bank`, `weaponbank`, `shieldbank`, `cosmeticsbank`, `paperdoll`, `spells`, `guild`, `guild_rank`, `guild_rank_string`, `quest`, `vars`, `fame`, `bestfame`,"
	...
	this->fame = GetRow<int>(row, "fame");
	this->bestfame = GetRow<int>(row, "bestfame");
	...
	this->mindam = 0;
	this->maxdam = 0;
	this->element_attack = 0; // Elemental attack
	this->element_weakness = 0; // Elemental weakness
	...
	this->weaponbank = ItemUnserialize(row["weaponbank"]);
	this->shieldbank = ItemUnserialize(row["shieldbank"]);
	this->cosmeticsbank = ItemUnserialize(row["cosmeticsbank"]);
	this->paperdoll = DollUnserialize(row["paperdoll"]);
	this->spells = SpellUnserialize(row["spells"]);

	this->player = 0;

	this->weaponbankopen = false;
	this->shieldbankopen = false;
	this->cosmeticsopen = false;
...
static bool character_equip_oneslot(Character *character, short item, unsigned char subloc, Character::EquipLocation slot)
{
	...
	{
		character->olditem = character->paperdoll[slot];

		if(character->olditem == character->paperdoll[Character::Weapon] || character->olditem == character->paperdoll[Character::Shield])
		{
			if(character->olditem != item)
			{
				PacketBuilder reply(PACKET_PAPERDOLL, PACKET_SWAP, 52);
				reply.AddShort(character->PlayerID());
				reply.AddChar(SLOT_CLOTHES);
				reply.AddChar(0); // sound

				reply.AddShort(character->paperdoll[Character::Boots]);
				reply.AddShort(character->paperdoll[Character::Armor]);
				reply.AddShort(character->paperdoll[Character::Hat]);
				reply.AddShort(character->paperdoll[Character::Weapon]);
				reply.AddShort(character->paperdoll[Character::Shield]);
				reply.AddShort(item);
				reply.AddThree(character->HasItem(item));
				reply.AddShort(0);
				reply.AddShort(character->olditem);
				reply.AddShort(character->HasItem(character->olditem));
				...
				character->AddItem(character->olditem, 1);

				PacketBuilder builder(PACKET_ITEM, PACKET_GET, 9);
				builder.AddShort(0); // UID
				builder.AddShort(character->olditem);
				builder.AddThree(1);
...
bool Character::Equip(short item, unsigned char subloc)
{
	...
	if (eif.type == EIF::Shield && eif.subtype != EIF::TwoHanded)
	{
...
std::string Character::PaddedGuildTag()
{
	...
	{
		int fame = this->fame;
		tag = util::to_string(this->fame);

		if (tag.length() <= 2)
		{
			tag.insert(tag.begin(), 'F');
		}
		else if(tag.length() == 3) {}
		else if(tag.length() >= 4 < 6)
		{	
			tag = util::to_string(fame / 1000)  + "K";
		}
...
void Character::CalculateStats(bool trigger_quests)
{
	...
	this->mindam = 0;
	this->maxdam = 0;
	this->element_attack = 0; //elemental calc
	this->element_weakness = 0; //elemental calc
	this->accuracy = 0;
	this->evade = 0;
	this->armor = 0;
	this->maxsp = 0;
    ...
    if (this->HasItem(36)) // Socket Items Weapon
    {
    	std::list<int> allowed_list = ExceptUnserialize(this->world->socket_config["WeaponSocketIDs"]); 	
    	UTIL_FOREACH(this->weaponbank, it)
		{
			if (std::find(allowed_list.begin(), allowed_list.end(), it.id) != allowed_list.end())
        	{   
			this->mindam += std::stoi(this->world->socket_config[util::to_string(it.id) + ".mindam"]);
			this->maxdam += std::stoi(this->world->socket_config[util::to_string(it.id) + ".maxdam"]);
			this->accuracy += std::stoi(this->world->socket_config[util::to_string(it.id) + ".accuracy"]);
			this->evade += std::stoi(this->world->socket_config[util::to_string(it.id) + ".evade"]);
			this->armor += std::stoi(this->world->socket_config[util::to_string(it.id) + ".armor"]);
			this->maxhp += std::stoi(this->world->socket_config[util::to_string(it.id) + ".maxhp"]);
			this->maxtp += std::stoi(this->world->socket_config[util::to_string(it.id) + ".maxtp"]);
			}
		}		
    }

    if (this->HasItem(37)) // Socket Items Shield
    {
    	std::list<int> allowed_list = ExceptUnserialize(this->world->socket_config["ShieldSocketIDs"]);
    	UTIL_FOREACH(this->shieldbank, it)
		{
			if (std::find(allowed_list.begin(), allowed_list.end(), it.id) != allowed_list.end())
        	{  
			this->mindam += std::stoi(this->world->socket_config[util::to_string(it.id) + ".mindam"]);
    ...
    if (this->HasItem(53)) // Costumes
    {
    	UTIL_FOREACH(this->cosmeticsbank, it)
			{
				if(this->gender == this->world->eif->Get(it.id).costume_gender)
				{
					if(this->cosmetic_paperdoll[Character::Armor] != this->world->eif->Get(it.id).costume_graphic)
					this->Dress(Character::Armor, this->world->eif->Get(it.id).costume_graphic);
				}
				else
				{
					if(this->world->eif->Get(it.id).costume_gender == 2)
					{
						if(this->cosmetic_paperdoll[Character::Boots] != this->world->eif->Get(it.id).costume_graphic)
						this->Dress(Character::Boots, this->world->eif->Get(it.id).costume_graphic);
					}
					if(this->world->eif->Get(it.id).costume_gender == 3)
					{
						if(this->cosmetic_paperdoll[Character::Shield] != this->world->eif->Get(it.id).costume_graphic)
						{
							if(this->world->eif->Get(it.id).costume_graphic == 999 && this->cosmetic_paperdoll[Character::Shield] != 0)
							{
								this->Dress(Character::Shield, 0);
							}
							else
							{
								this->Dress(Character::Shield, this->world->eif->Get(it.id).costume_graphic);
							}
						}
					}					
				}
	...
	UTIL_FOREACH(this->paperdoll, i)
	{
		if (i)
		{
			...
			this->element_attack += item.element_attack; // Elemental
...
void Character::DropInv(Character *killer)
{
...
void Character::Reset()
{
	if (this->boosttimer <= 0) {
		...
		PacketBuilder reply(PACKET_STATSKILL, PACKET_JUNK, 12);
    ...
    {
        this->ServerMsg("You cannot reset while buffed.");
    }
}
...
void Character::DeathRespawn()
{
	...
	if (this->world->config["Deadly"] && this->mapid != 4 && this->mapid != 14 && this->mapid != 17) // Trials Map IDs
	{
		this->DropInv(nullptr);
		if (this->bestfame < this->fame)
		{
			this->bestfame = this->fame;
		}
		this->fame = 0;
		this->race = Skin(4);
		this->indungeon = false;		
...
void Character::FormulaVars(std::unordered_map<std::string, double> &vars, std::string prefix)
{
	...
	v(weight) v(maxweight) v(karma) v(mindam) v(maxdam) v(element_attack) v(element_weakness)
...
void Character::UndoBuff()
{
    if (this->boosttimer > 0)
    {
        ...
        this->ServerMsg("You are no longer buffed!");
    }
    else
    {
        this->ServerMsg("You are not buffed!");
    }
}
...
void Character::Save()
{
	...
	this->world->db.Query("UPDATE `characters` SET `title` = '$', `home` = '$', `fiance` = '$', `partner` = '$', `admin` = #, `class` = #, `gender` = #, `race` = #, "
		"`hairstyle` = #, `haircolor` = #, `map` = #, `x` = #, `y` = #, `direction` = #, `level` = #, `exp` = #, `fame` = #, `bestfame` = #, `hp` = #, `tp` = #, "
		"`str` = #, `int` = #, `wis` = #, `agi` = #, `con` = #, `cha` = #, `statpoints` = #, `skillpoints` = #, `karma` = #, `sitting` = #, `hidden` = #, "
		"`nointeract` = #, `bankmax` = #, `goldbank` = #, `usage` = #, `inventory` = '$', `bank` = '$', `weaponbank` = '$', `shieldbank` = '$', `cosmeticsbank` = '$', `paperdoll` = '$', "
		...
		this->hairstyle, this->haircolor, this->mapid, this->x, this->y, int(this->direction), this->level, this->exp, this->fame, this->bestfame, this->hp, this->tp,
		...
		nointeract, this->bankmax, this->goldbank, this->Usage(), ItemSerialize(this->inventory).c_str(), ItemSerialize(this->bank).c_str(), ItemSerialize(this->weaponbank).c_str(), ItemSerialize(this->shieldbank).c_str(), ItemSerialize(this->cosmeticsbank).c_str(),
