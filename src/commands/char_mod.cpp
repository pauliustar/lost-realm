...
void SetX(const std::vector<std::string>& arguments, Command_Source* from, std::string set)
{
	...
	else
	{
		...
		else if (set == "fame") (fame = true, victim->fame) = std::min(std::max(util::to_int(arguments[1]), 0), int(from->SourceWorld()->config["MaxExp"]));		
...
COMMAND_HANDLER_REGISTER(char_mod)
	...
	Register({"setfame", {"victim", "value"}, {}, 4}, std::bind(SetX, _1, _2, "fame"), CMD_FLAG_DUTY_RESTRICT);
