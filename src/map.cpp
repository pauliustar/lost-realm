...
void Map::Attack(Character *from, Direction direction)
{
    ...
    if (from->paperdoll[Character::Weapon] > 0)
    {
        ...
        if (from->paperdoll[Character::Weapon] == util::to_int(this->world->mining_config["MiningWeapon"]))
        {
	...
	else if (this->world->eif->Get(from->paperdoll[Character::Weapon]).id == 24)
	{
		range = 4;
	}		
	...
	for (int i = 0; i < range; ++i)
	{
		...
		UTIL_FOREACH(this->npcs, npc)
		{
			 ...
			 && npc->alive && npc->x == target_x && npc->y == target_y)
			{
				...
				int player_element_attack = wepdata.element_attack;

				int element_attack = npc->ENF().element_attack;
				int element_weakness = npc->ENF().element_weakness;

				int element_attack_dmg = npc->ENF().element_attack_dmg;
				int element_weakness_dmg = npc->ENF().element_weakness_dmg;
				...
				{
					if (player_element_attack > element_weakness_dmg)
					{
						int element_chance = util::rand(1, 100);
						element_amount = player_element_attack / element_weakness_dmg;
						if (element_amount <= 0)
						{
							element_amount = 0;
							advantage = 0;
				...
				if(from->HasItem(25) > 0 && this->world->eif->Get(from->paperdoll[Character::Weapon]).subtype == EIF::Ranged)
				{
					critical = false;
				}
				else if(from->clas == 4)
				...
				*/
				...
				else if (rand < hit_rate && critical == true)
				{
					// Critical Effect
					from->CancelSpell();
					...
					UTIL_FOREACH(from->map->characters, updatecharacter)
					{
						if (updatecharacter != from && from->InRange(updatecharacter))
						{
							updatecharacter->Send(builder);
						}
					}

					from->map->SpellAttack(from, npc, 1);
				}
				if(advantage == 1)
		    	{


		    		this->world->ServerMsg("Player Element Amount " + util::to_string(element_amount));
		    		...
		    		switch (player_element) {
		    			case EIF::Light:
		    				effect = 22; //Light
		    				this->world->ServerMsg("effect " + util::to_string(effect));
		    				break;
		    			case EIF::Dark:
		    				effect = 31; //Dark
		    				this->world->ServerMsg("effect " + util::to_string(effect));
		    		...
		    		UTIL_FOREACH(this->characters, character)
					{
						...
						builder.AddChar(target_x);	
						builder.AddChar(target_y);
				...
				//Arrow Effect (Requires Ranged weapon)

				if (shielddata.id == 26 && wepdata.id == 27 || wepdata.id == 95)
				{
					double hit_chance = 0.30;
					if (rand < hit_chance)
					{
						if(from->HasItem(25) > 0)
						{
							amount = 2 * amount;
							...
							from->map->SpellAttack(from, npc, 3);	
						}
						else
						{
							from->StatusMsg("Special Attack failed! No ammo to shoot.");
						}										
					}
				}

				//Soul Attack (Death Star)
				if (wepdata.id == 24)
				{
					if(from->HasItem(24) > 0)
					{
						from->DelItem(24, 1);
					...
					{
						if(from->Unequip(24, 0))
						{
							PacketBuilder reply(PACKET_PAPERDOLL, PACKET_REMOVE, 43);
							reply.AddShort(from->PlayerID());
							reply.AddChar(SLOT_CLOTHES);
							reply.AddChar(0); // sound
							from->AddPaperdollData(reply, "BAHWS");
							reply.AddShort(24);
							reply.AddChar(0);
							reply.AddShort(from->maxhp);
							reply.AddShort(from->maxtp);
							reply.AddShort(from->display_str);
							reply.AddShort(from->display_intl);
							reply.AddShort(from->display_wis);
							reply.AddShort(from->display_agi);
							reply.AddShort(from->display_con);
							reply.AddShort(from->display_cha);
							reply.AddShort(from->mindam);
							reply.AddShort(from->maxdam);
							reply.AddShort(from->accuracy);
							reply.AddShort(from->evade);
							reply.AddShort(from->armor);
...
void Map::Mine(Character *from)
{
    ...
    {
        if ((double(util::rand(0,10000)) / 100.0) < checkdrop->chance)
    ...
    if (drop)
    {
    	if (drop->item == 0)
    	{
    		from->StatusMsg(from->world->i18n.Format("Material extraction failed!"));
    	}
    	else
    	{
	        from->StatusMsg(from->world->i18n.Format("Successfully extracted: ", this->world->eif->Get(drop->item).name));
...
void Map::SpellSelf(Character *from, unsigned short spell_id)
{
	...
	int hpgain = spell.hp;
	int tpdecrease = spell.tp;
	int extra_hp;

	// Spell levels
	UTIL_IFOREACH(from->spells, list_item)
	{
		// Rejuvenation
		if (list_item->id == 2)
		{
			hpgain += list_item->level;
			extra_hp = hpgain;
			tpdecrease += list_item->level;
		}
	}

	if (!spell || spell.type != ESF::Heal || from->tp < tpdecrease)
		return;
...
void Map::QuestHeal(Character *from, unsigned short spell_id, int amount)
{
	const ESF_Data& spell = from->world->esf->Get(spell_id);
...
void Map::SpellAttack(Character *from, NPC *npc, unsigned short spell_id)
{
	...
	if ((npc->ENF().type == ENF::Passive || npc->ENF().type == ENF::Aggressive) && npc->alive)
	{
		...
		if (spell_id == 1 || spell_id == 3) // Critical Spell Damange // Double Hit Spell Damage
		{
			amount = 0;
		}
