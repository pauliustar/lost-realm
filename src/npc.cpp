...
void NPC::Spawn(NPC *parent)
{
	...
	{
	    std::string strings[] = {
		    "A brief glimmer of darkness, your reward awaits.",
		    "A fleeting shadow bears riches for the bold.",
		    "From shadows, a gift awaits, swiftly taken.",
		    "The darkness yields a reward, but time is short.",
		    "Treasure from the abyss, seize it before it fades."
	    };

	    // Generate a random index
	    int randomIndex = std::rand() % 5;

	    // Get the randomly chosen string
	    std::string chosenString = strings[randomIndex];		
		this->ShowDialog(chosenString);
	}
	if (id == 35)
	{
		this->ShowDialog("You will pay for this!");
	}
	else if (id == 36)
	{
		this->ShowDialog("No mercy for you! Die!");
	}
	else if (id == 53)
	{
		this->ShowDialog("Intruder alert! You won't see another day after I slash you in two!");
	}	
	else if (id == 56)
	{
	    std::string strings[] = {
		    "Feeling lucky today? Luck's a fickle thing when you're on my menu.",
		    "Fortune favors the bold, they say. But does it favor you?",
		    "Luck may smile upon you, but beware the twist of fate that I bring.",
		    "Luck's coin has two sides, adventurer. Today, it's my side.",
		    "Roll the dice of destiny, but remember, I'm the house, and the house always wins."
	    };
...
void NPC::Act()
{
	...
	if (this->ENF().type == ENF::Passive || this->ENF().type == ENF::Aggressive)
	{
		UTIL_FOREACH_CREF(this->damagelist, opponent)
		{
			...
			if ((distance < attacker_distance) || (distance == attacker_distance && opponent->attacker->clas == 2))
			{
				attacker = opponent->attacker;
				attacker_damage = opponent->damage;
				attacker_distance = distance;
			}
			else if ((distance < attacker_distance) || (distance == attacker_distance && opponent->damage > attacker_damage))
			{
				attacker = opponent->attacker;
				attacker_damage = opponent->damage;
				attacker_distance = distance;
...
void NPC::Killed(Character *from, int amount, int spell_id)
{
	...
	int deathmap = this->map->id;
	int deathx = this->x;
	int deathy = this->y;

	if (id != 7 && id != 15 && id != 45 && id != 55 && id != 56 && id != 34 && id != 35 && id != 52)
	{
		// Chest Roll
		double roll_soul = util::rand(0.0, 100.0); 

		int soul_item = static_cast<int>(this->map->world->config["SoulItem"]);

		if (roll_soul >= 0.0 && roll_soul < 2.0)
		{
			this->map->world->SpawnDungeonNPC(43, deathmap, deathx, deathy, 1);
		}	
	}		
	if (this->id == 34)
	{
		...
		if (roll_soul >= 30.0 && roll_soul < 60.0)
		{
			this->map->world->SpawnDungeonNPC(35, deathmap, deathx, deathy, 3);
			UTIL_FOREACH(this->map->characters, character)
			{
				PacketBuilder builder(PACKET_EFFECT, PACKET_AGREE);
				builder.AddChar(deathx);	
				builder.AddChar(deathy);
				builder.AddThree(6);


				if (from->InRange(this))
				{
					from->Send(builder);
				}
			}
	...
	{	
		...
		if (roll_soul >= 45.0 && roll_soul < 60.0)
		{		
			this->map->world->SpawnDungeonNPC(36, deathmap, deathx, deathy, 3);	
	...
	{	
		...
		if (roll_soul >= 0.0 && roll_soul < 3.0)
		{		
			this->map->world->SpawnDungeonNPC(53, deathmap, deathx, deathy, 3);	
	...
	{	
		...
		if (roll_soul >= 0.0 && roll_soul < 35.0)
		{		
			this->map->world->SpawnDungeonNPC(56, deathmap, deathx, deathy, 2);	     
		}
	...
	if(this->alive == false)
	{
		...
		if (roll_soul >= 30.0 && roll_soul < 60.0 && from->mapid != 1)
		{
			from->AddItem(soul_item, 1);

			PacketBuilder reply(PACKET_ITEM, PACKET_GET, 9);
			reply.AddShort(0); // UID
			reply.AddShort(soul_item);
			reply.AddThree(1);
			reply.AddChar(from->weight);
			reply.AddChar(from->maxweight);
			from->Send(reply);
		}
		// Death Counter & Effect
		if (from->mapid == 1 && this->map->world->EscapeDeath == true)
		{
			this->map->world->deathcounter++;

			if (this->map->world->deathcounter < 6)
			{
				PacketBuilder builder(PACKET_EFFECT, PACKET_AGREE);
				builder.AddChar(16);	
				builder.AddChar(13);
    ...
    if (from->party) // Party kills
    {
            ...
            { 
                UTIL_FOREACH(member->quests, q)
                {
                    ...
                    // add condition to check if member is on the same map as the killed NPC
...
void NPC::Attack(Character *target)
{
	...
	const EIF_Data& wepdata = this->map->world->eif->Get(target->paperdoll[Character::Weapon]);
	int player_element = wepdata.element;

	int element_attack = this->ENF().element_attack;
	int element_weakness = this->ENF().element_weakness;

	int element_attack_dmg = this->ENF().element_attack_dmg;
	int element_weakness_dmg = this->ENF().element_weakness_dmg;

	int advantage = 0;
	if (player_element == element_weakness)
	 	advantage = 1;
	...
	bool critical = rand < static_cast<double>(this->map->world->config["CriticalRate"]);
	...
	if (critical == true && rand < hit_rate)
	{
		// Critical Effect

		this->ShowDialog("*Critical Hit!*");

		PacketBuilder builder(PACKET_AVATAR, PACKET_ADMIN, 12);
		builder.AddShort(0);
		builder.AddShort(target->PlayerID());
		builder.AddThree(amount);
		builder.AddChar(target->direction);
		builder.AddChar(util::clamp<int>(double(target->hp) / double(target->maxhp) * 100.0, 0, 100));
		builder.AddChar(target->hp == 0);
		builder.AddShort(1);

		UTIL_FOREACH(this->map->characters, character)
		{
			if (target->InRange(character))
				character->Send(builder);
		}
	}
	...
	// TODO: Monster elemental attacks
	/*else
	{
		...
		{
			PacketBuilder builder(PACKET_EFFECT, PACKET_AGREE);
			builder.AddChar(target->x);	
			builder.AddChar(target->y);
			builder.AddThree(13);


			if (character->InRange(target))
			{
				character->Send(builder);
			}
		}
	//amount += element_power;
	this->map->world->ServerMsg("NPC Element Amount " + util::to_string(element_amount));
	}*/
...
void NPC::ShowDialog(std::string message)
{
    ...
    if (message != "0")
    {
    	PacketBuilder builder(PACKET_NPC, PACKET_PLAYER, 255);
    	builder.AddByte(255);
        builder.AddByte(255);
        builder.AddChar(this->index);
...
void NPC::FormulaVars(std::unordered_map<std::string, double> &vars, std::string prefix)
{
	...
	vd(element_attack) vd(element_weakness)
}
