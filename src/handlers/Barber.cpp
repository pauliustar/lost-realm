...
void Barber_Open(Character *character, PacketReader &reader)
{
	...
	UTIL_FOREACH(character->map->npcs, npc)
	{
		if (npc->index == id && npc->ENF().type == ENF::Barber)
		{
			...
			PacketBuilder reply(PACKET_BARBER, PACKET_OPEN, 10);
			reply.AddInt(0); // Session token
			reply.AddShort(20); // Add new field: maxHairModals
            reply.AddShort(5); // Add new field: basePrice
            reply.AddShort(5); // Add new field: pricePerLevel
